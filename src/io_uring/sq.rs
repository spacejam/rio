...
impl Sq {
    pub(crate) fn new(params: &io_uring_params, ring_fd: i32) -> io::Result<Sq> {
        ...
        Ok(unsafe {
            Sq {
                ...
                khead: &*(sq_ring_ptr.add(params.sq_off.head as usize) as *const AtomicU32),
                ktail: &*(sq_ring_ptr.add(params.sq_off.tail as usize) as *const AtomicU32),
                kring_mask: &*(sq_ring_ptr.add(params.sq_off.ring_mask as usize) as *const u32),
                kflags: &*(sq_ring_ptr.add(params.sq_off.flags as usize) as *const AtomicU32),
                kdropped: &*(sq_ring_ptr.add(params.sq_off.dropped as usize) as *const AtomicU32),
    ...
    pub(crate) fn submit_all(&mut self, ring_flags: u32, ring_fd: i32) -> u64 {
        let submitted = if ring_flags & IORING_SETUP_SQPOLL == 0 {
            ...
            flushed
        } else if self.kflags.load(Acquire) & IORING_SQ_NEED_WAKEUP != 0 {
